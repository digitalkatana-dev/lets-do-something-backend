		...
		req?.body?.invitedGuests?.forEach(async (item) => {
			if (item.notify === 'sms') {
				await twilioClient.messages.create({
					body: `You've been invited to ${req?.body?.type} on ${req?.body?.date} at ${req?.body?.time} by ${req?.user?.firstName}. Click here -> http://localhost:3000 to RSVP!`,
...
router.post('/events/invite', requireAuth, async (req, res) => {
	...
	try {
		if (isPhone(guest)) {
			await twilioClient.messages.create({
				body: `You've been invited to ${type} on ${date} at ${time} by ${req?.user?.firstName}. Click here -> http://localhost:3000 to RSVP!`,
				from: process.env.TWILIO_NUMBER,
				to: `+1${guest}`,
			});
